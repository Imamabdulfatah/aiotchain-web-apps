...
function ManagePathCategoriesPage() {
  ...
  const loadCategories = async () => {
    try {
      const data = await fetchAPI<PathCategory[]>("/admin/path-categories");
      setCategories(data);
    } catch (err: any) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchAPI("/admin/path-categories", {
        method: "POST",
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      const token = getToken();
      await fetchAPI(`/admin/path-categories/${id}`, {
        method: "PUT",
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Hapus kategori ini? Pastikan tidak ada alur belajar yang menggunakannya.")) return;
    try {
      const token = getToken();
      await fetchAPI(`/admin/path-categories/${id}`, {
        method: "DELETE",
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="space-y-1">
        <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Kategori Alur Belajar</h1>
        <p className="text-slate-500 font-medium">Kelola kategori teknologi untuk alur belajar (Javascript, Prompt Engineering, dll).</p>
      </div>
...
export default withAuth(ManagePathCategoriesPage);
